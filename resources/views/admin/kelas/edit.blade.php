...
<style>
    ...
    .btn-submit {
        ...
        cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(117,22,46,0.2);
    }
    .btn-submit:hover { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(117,22,46,0.28); }
...
<div class="page-header">
    ...
    <div>
        <div class="page-header__title">Edit Kelas</div>
        <div class="page-header__sub">Ubah data kelas {{ $kelas->nama }}</div>
    </div>
</div>
...
<div class="form-card">
    <div class="form-card__header">
        ...
        <div>
            <div class="form-card__title">Edit Data Kelas</div>
            <div class="form-card__sub">Perbarui informasi kelas</div>
        </div>
    </div>
    ...
    <div class="form-card__body">
        <form method="POST" action="{{ route('admin.kelas.update', $kelas) }}">
            ...
            <div class="form-group">
                ...
                <div class="form-grid-3">
                    ...
                    <div>
                        ...
                        <select id="jurusan_select" name="jurusan"
                            ...
                            <option value="TM"  {{ old('jurusan', $kelas->jurusan) === 'TM'  ? 'selected' : '' }}>TM</option>
                            <option value="AKL" {{ old('jurusan', $kelas->jurusan) === 'AKL' ? 'selected' : '' }}>AKL</option>
                            <option value="PB"  {{ old('jurusan', $kelas->jurusan) === 'PB'  ? 'selected' : '' }}>PB</option>
                ...
                <input type="hidden" id="nama" name="nama" value="{{ old('nama', $kelas->nama) }}">
...
<script>
// Parse nama kelas yang sudah ada → isi ke 3 dropdown
window.addEventListener('DOMContentLoaded', function () {
    const namaAwal = '{{ $kelas->nama }}';

    // Tingkat: X, XI, XII
    const tingkatOpts = ['XII', 'XI', 'X']; // urutan panjang dulu agar XII tidak match X
    let sisaNama = namaAwal.trim();
    let detectedTingkat = '';

    for (const t of tingkatOpts) {
        if (sisaNama.startsWith(t)) {
            detectedTingkat = t;
            sisaNama = sisaNama.slice(t.length).trim();
            break;
        }
    }

    // Jurusan: cocokkan dari daftar
    const jurusanOpts = ['RPL', 'TKJ', 'TM', 'AKL', 'PB', 'DKV'];
    let detectedJurusan = '';
    for (const j of jurusanOpts) {
        if (sisaNama.startsWith(j)) {
            detectedJurusan = j;
            sisaNama = sisaNama.slice(j.length).trim();
            break;
        }
    }

    // Nomor: sisa string
    const detectedNomor = sisaNama;

    // Set ke dropdown & input
    if (detectedTingkat) document.getElementById('tingkat').value = detectedTingkat;
    if (detectedJurusan) document.getElementById('jurusan_select').value = detectedJurusan;
    if (detectedNomor)   document.getElementById('nomor').value = detectedNomor;
