...
<style>
    ...
    .pw-note svg { width: 16px; height: 16px; flex-shrink: 0; margin-top: 1px; }
</style>

<!-- Header -->
<div class="page-header">
    <div>
        <div class="page-header__title">Edit Guru BK</div>
        <div class="page-header__sub">Perbarui data: {{ $guru->name }}</div>
    </div>
...
<div class="card">
    <form method="POST" action="{{ route('admin.guru-bk.update', $guru) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="number" name="tahun_mulai_bertugas"
                       value="{{ old('tahun_mulai_bertugas', $guru->tahun_mulai_bertugas) }}"
        ...
        <div class="form-group full">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 10px; margin-top: 4px;">
                ...
                <label style="display: flex; align-items: center; gap: 8px; padding: 10px 14px;
                            border: 1.5px solid {{ in_array($k->id, old('kelas_ids', $kelasBindaan)) ? '#16a34a' : '#e2e8f0' }};
                            border-radius: 10px; cursor: pointer; font-weight: 500; font-size: 0.82rem;
                            background: {{ in_array($k->id, old('kelas_ids', $kelasBindaan)) ? '#f0fdf4' : '#fafbff' }};
                            color: {{ in_array($k->id, old('kelas_ids', $kelasBindaan)) ? '#166534' : 'var(--navy-darkest)' }};
                            transition: all 0.2s;"
                    ...
                    <span style="width:18px;height:18px;border-radius:4px;border:2px solid currentColor;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:12px;font-weight:700;">
                        <span class="check-icon" style="display:{{ in_array($k->id, old('kelas_ids', $kelasBindaan)) ? 'block' : 'none' }}">✓</span>
                    </span>
                    <input type="checkbox" name="kelas_ids[]" value="{{ $k->id }}"
                        {{ in_array($k->id, old('kelas_ids', $kelasBindaan)) ? 'checked' : '' }}
                        style="display:none;">
                    {{ $k->nama }}
                </label>
                @empty
