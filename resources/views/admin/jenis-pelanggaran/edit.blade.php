...
<div class="page-header">
    <div>
        <div class="page-header__title">Edit Jenis Pelanggaran</div>
        <div class="page-header__sub">Perbarui data — {{ $jenisPelanggaran->nama }}</div>
    </div>
...
<form method="POST" action="{{ route('admin.jenis-pelanggaran.update', $jenisPelanggaran) }}">
    @csrf
    @method('PUT')
    <div class="card">
        ...
        <div class="form-grid">
            <div class="form-group full">
                ...
                <input type="text" name="nama" value="{{ old('nama', $jenisPelanggaran->nama) }}" placeholder="Contoh: Terlambat masuk sekolah">
            ...
            <div class="form-group full">
                ...
                <select name="kategori">
                    ...
                    <option value="ringan" {{ old('kategori', $jenisPelanggaran->kategori) == 'ringan' ? 'selected' : '' }}>Ringan (poin 1-20)</option>
                    <option value="sedang" {{ old('kategori', $jenisPelanggaran->kategori) == 'sedang' ? 'selected' : '' }}>Sedang (poin 21-50)</option>
                    <option value="berat"  {{ old('kategori', $jenisPelanggaran->kategori) == 'berat'  ? 'selected' : '' }}>Berat (poin 51-100)</option>
                </select>
                @error('kategori') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="form-group">
                <label>Poin Sanksi <span class="req">*</span></label>
                <input type="number" name="poin" value="{{ old('poin', $jenisPelanggaran->poin) }}" min="1" max="100">
            ...
            <div class="form-group">
                ...
                <div class="toggle-wrapper" style="margin-top:8px;">
                    <label class="toggle">
                        <input type="checkbox" name="is_aktif" {{ old('is_aktif', $jenisPelanggaran->is_aktif) ? 'checked' : '' }}>
            ...
            <div class="form-group full">
                ...
                <textarea name="deskripsi" rows="3" placeholder="Penjelasan singkat...">{{ old('deskripsi', $jenisPelanggaran->deskripsi) }}</textarea>
