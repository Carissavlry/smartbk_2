...
<div class="container mx-auto px-4 py-6 max-w-2xl">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Tambah Mutasi Siswa</h1>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.mutasi-siswa.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="jenis_mutasi" id="jenis_mutasi" required onchange="toggleFields()"
                    ...
                    <option value="masuk" {{ old('jenis_mutasi') == 'masuk' ? 'selected' : '' }}>Mutasi Masuk</option>
                    <option value="keluar" {{ old('jenis_mutasi') == 'keluar' ? 'selected' : '' }}>Mutasi Keluar</option>
                    <option value="internal" {{ old('jenis_mutasi') == 'internal' ? 'selected' : '' }}>Mutasi Internal</option>
                </select>
            ...
            <div id="field_masuk" class="hidden">
                ...
                <div class="mb-4">
                    ...
                    <select name="kelas_tujuan_id"
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}" {{ old('kelas_tujuan_id') == $k->id ? 'selected' : '' }}>
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div id="field_keluar" class="hidden">
                <div class="mb-4">
                    ...
                    <select name="kelas_asal_id"
                        ...
                        <option value="">-- Pilih Kelas --</option>
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}" {{ old('kelas_asal_id') == $k->id ? 'selected' : '' }}>
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="alasan" rows="3" placeholder="Tuliskan alasan mutasi (opsional)"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('alasan') }}</textarea>
            </div>
