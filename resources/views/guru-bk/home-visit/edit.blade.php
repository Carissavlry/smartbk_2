...
<style>
    ...
    .foto-hint { font-size:0.72rem; color:#94a3b8; margin-top:4px; margin-bottom:8px; }
    .foto-item { display:flex; flex-direction:column; align-items:center; gap:5px; position:relative; flex-shrink:0; width:90px; }
    .foto-item img { width:90px; height:90px; object-fit:cover; border-radius:10px; border:1.5px solid #e2e8f0; display:block; }
    .foto-item span { font-size:0.7rem; color:#64748b; font-weight:600; white-space:nowrap; }
    .foto-item-baru img { border-color:#bfdbfe; }
    .foto-item-baru span { color:#3b82f6; }
    .foto-wrap { display:flex; flex-wrap:wrap; gap:12px; margin-top:10px; }
    .btn-hapus-foto { position:absolute; top:-6px; right:-6px; width:20px; height:20px; background:#ef4444; color:white; border:none; border-radius:50%; font-size:0.65rem; cursor:pointer; display:flex; align-items:center; justify-content:center; font-weight:700; line-height:1; padding:0; z-index:2; }
</style>

<div style="margin-bottom:20px;">
    <a href="{{ route('guru-bk.home-visit.show', $homeVisit) }}" class="btn-back">
...
<form method="POST" action="{{ route('guru-bk.home-visit.update', $homeVisit) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="card-body">
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <div class="nomor-surat-badge">{{ $homeVisit->nomor_surat ?? '-' }}</div>
                </div>
                <div class="form-group">
                    <label>Siswa</label>
                    <input type="text" class="form-control" disabled
                           value="{{ $homeVisit->siswa->name }} — {{ $homeVisit->siswa->kelas->nama ?? '-' }}">
                </div>
                <div class="form-group">
                    <label>Tanggal Kunjungan <span class="req">*</span></label>
                    <input type="date" name="tanggal" class="form-control"
                           value="{{ old('tanggal', $homeVisit->tanggal->format('Y-m-d')) }}" required>
                </div>
                <div class="form-group">
                    <label>Jam Mulai <span class="req">*</span></label>
                    <input type="time" name="jam_mulai" class="form-control"
                           value="{{ old('jam_mulai', $homeVisit->jam_mulai) }}" required>
                </div>
                <div class="form-group">
                    <label>Jam Selesai <span class="req">*</span></label>
                    <input type="time" name="jam_selesai" class="form-control"
                           value="{{ old('jam_selesai', $homeVisit->jam_selesai) }}" required>
                </div>
                <div class="form-group">
                    <label>Yang Menemani Guru BK</label>
                    <input type="text" name="yang_menemani" class="form-control"
                           value="{{ old('yang_menemani', $homeVisit->yang_menemani) }}"
                ...
                <div class="form-group">
                    ...
                    <select name="status_kehadiran_ortu" class="form-control" required>
                        <option value="Ada" {{ old('status_kehadiran_ortu', $homeVisit->status_kehadiran_ortu) == 'Ada' ? 'selected' : '' }}>Ada</option>
                        <option value="Tidak Ada" {{ old('status_kehadiran_ortu', $homeVisit->status_kehadiran_ortu) == 'Tidak Ada' ? 'selected' : '' }}>Tidak Ada</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input type="text" class="form-control" disabled
                        value="{{ $homeVisit->siswa->alamat ?? 'Belum diisi — hubungi Admin' }}"
                        style="background:#f8fafc; color:{{ $homeVisit->siswa->alamat ? '#1e293b' : '#f59e0b' }};">
                    <input type="hidden" name="alamat" value="{{ $homeVisit->siswa->alamat ?? '' }}">
                </div>
                <div class="form-group">
                    <label>Nama Orang Tua / Wali</label>
                    <input type="text" class="form-control" disabled
                        value="{{ $homeVisit->siswa->nama_ortu ?? 'Belum diisi — hubungi Admin' }}"
                        style="background:#f8fafc; color:{{ $homeVisit->siswa->nama_ortu ? '#1e293b' : '#f59e0b' }};">
                    <input type="hidden" name="nama_ortu" value="{{ $homeVisit->siswa->nama_ortu ?? '' }}">
                </div>
                <div class="form-group">
                    <label>No HP Orang Tua</label>
                    <input type="text" class="form-control" disabled
                        value="{{ $homeVisit->siswa->no_hp_ortu ?? 'Belum diisi — hubungi Admin' }}"
                        style="background:#f8fafc; color:{{ $homeVisit->siswa->no_hp_ortu ? '#1e293b' : '#f59e0b' }};">
                    <input type="hidden" name="no_hp_ortu" value="{{ $homeVisit->siswa->no_hp_ortu ?? '' }}">
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <textarea name="tujuan" class="form-control" required
                        placeholder="Contoh: Menindaklanjuti ketidakhadiran siswa 3 hari berturut-turut (alasan utama kunjungan dilakukan)">{{ old('tujuan', $homeVisit->tujuan) }}</textarea>
                </div>
                <div class="form-group full">
                    <label>Kondisi Lingkungan Rumah</label>
                    <textarea name="kondisi_lingkungan" class="form-control"
                        placeholder="Contoh: Rumah layak huni, orang tua bekerja sebagai buruh, siswa diasuh nenek (gambaran situasi keluarga saat kunjungan)">{{ old('kondisi_lingkungan', $homeVisit->kondisi_lingkungan) }}</textarea>
                </div>
                <div class="form-group full">
                    <label>Hasil Kunjungan</label>
                    <textarea name="hasil" class="form-control"
                        placeholder="Contoh: Orang tua belum mengetahui absensi siswa, siswa mengaku malas karena masalah pertemanan (temuan faktual saat kunjungan)">{{ old('hasil', $homeVisit->hasil) }}</textarea>
                </div>
                <div class="form-group full">
                    <label>Kesimpulan</label>
                    <textarea name="kesimpulan" class="form-control"
                        placeholder="Contoh: Siswa membutuhkan pendampingan khusus akibat kurangnya perhatian orang tua di rumah (ringkasan keseluruhan kunjungan)">{{ old('kesimpulan', $homeVisit->kesimpulan) }}</textarea>
                </div>
                <div class="form-group full">
                    <label>Rekomendasi</label>
                    <textarea name="rekomendasi" class="form-control"
                        placeholder="Contoh: Koordinasi dengan wali kelas dan orang tua untuk membuat jadwal belajar di rumah (saran/solusi yang diusulkan saat kunjungan)">{{ old('rekomendasi', $homeVisit->rekomendasi) }}</textarea>
                </div>
                <div class="form-group full">
                    <label>Tindak Lanjut</label>
                    <textarea name="tindak_lanjut" class="form-control"
                        placeholder="Contoh: Jadwalkan sesi konseling Senin depan, hubungi orang tua via telepon H+3 untuk konfirmasi (aksi nyata yang akan dilakukan setelah kunjungan)">{{ old('tindak_lanjut', $homeVisit->tindak_lanjut) }}</textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <span class="card-header-title">Foto Dokumentasi ({{ $homeVisit->fotos->count() }}/10)</span>
        </div>
        <div class="card-body">
            <div class="form-group">

                <label style="font-size:0.78rem; font-weight:600; color:#374151;">Upload Foto</label>

                @if($homeVisit->fotos->count() < 10)
                <input type="file" name="fotos[]" id="fotos" class="form-control"
                    accept="image/jpg,image/jpeg,image/png"
                    multiple onchange="previewFotos(this)"
                    style="margin-top:6px;">
                @endif

                <div class="foto-hint">
                    Sisa slot: {{ 10 - $homeVisit->fotos->count() }} foto. Format: JPG, JPEG, PNG. Maks 2MB per foto.
                </div>

                <div id="fotoInfo" style="padding:8px 14px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:8px; font-size:0.78rem; color:#1d4ed8; font-weight:600; margin-bottom:12px;">
                    ...
                    <span id="fotoInfoText">{{ $homeVisit->fotos->count() }} foto tersimpan. Anda dapat menambahkan <strong>{{ 10 - $homeVisit->fotos->count() }} foto lagi</strong>.</span>
                </div>

                {{-- SEMUA FOTO BERJEJER: existing + baru --}}
                <div class="foto-wrap">

                    @foreach($homeVisit->fotos as $index => $foto)
                    <div class="foto-item">
                        <button type="button" class="btn-hapus-foto"
                            onclick="hapusFotoExisting({{ $foto->id }}, '{{ route('guru-bk.home-visit.foto.destroy', $foto) }}', this)">✕</button>
                        <img src="{{ asset('storage/' . $foto->foto) }}" alt="Foto ke-{{ $index + 1 }}">
                        <span>Foto ke-{{ $index + 1 }}</span>
                    </div>
                    @endforeach

                    <div id="fotoPreviewBaru" style="display:contents;"></div>
        ...
        <div class="footer-actions">
            <a href="{{ route('guru-bk.home-visit.show', $homeVisit) }}" class="btn-cancel">Batal</a>
...
<script>
let selectedFiles = [];
let existingCount = {{ $homeVisit->fotos->count() }};
let existingCountLive = {{ $homeVisit->fotos->count() }};
let maxSlot = {{ 10 - $homeVisit->fotos->count() }};
...
function renderPreview() {
    const preview  = document.getElementById('fotoPreviewBaru');
    const infoText = document.getElementById('fotoInfoText');

    preview.innerHTML = '';

    const totalFoto = existingCount + selectedFiles.length;
    const sisaSlot  = 10 - totalFoto;

    if (selectedFiles.length === 0) {
        infoText.innerHTML = `${existingCount} foto tersimpan. Anda dapat menambahkan <strong>${sisaSlot} foto lagi</strong>.`;
        return;
    }

    if (sisaSlot === 0) {
        infoText.innerHTML = `✅ Total ${totalFoto} foto — batas maksimal tercapai.`;
    } else {
        infoText.innerHTML = `${existingCount} foto tersimpan + ${selectedFiles.length} foto baru = <strong>${totalFoto} foto</strong>. Anda dapat menambahkan <strong>${sisaSlot} foto lagi</strong>.`;
    }

    selectedFiles.forEach((file, index) => {
        const nomorUrut = existingCount + index + 1;
        const reader = new FileReader();
        reader.onload = e => {
            const wrapper = document.createElement('div');
            wrapper.className = 'foto-item foto-item-baru';
            ...
            img.alt = `Foto Baru ke-${nomorUrut}`;

            const label = document.createElement('span');
            label.textContent = `Foto Baru ke-${nomorUrut}`;
    ...
    function hapusFotoExisting(fotoId, url, btn) {
        if (!confirm('Hapus foto ini?')) return;

        const wrapper = btn.closest('.foto-item');
        ...
        .then(res => {
            if (res.ok || res.redirected) {
                wrapper.remove();
                // Update label semua foto existing
                document.querySelectorAll('#fotoPreviewBaru').forEach(() => {});
                // Update info
                existingCountLive--;
                renderPreview();
            } else {
