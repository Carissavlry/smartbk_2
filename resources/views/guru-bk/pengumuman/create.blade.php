...
<div style="max-width:720px;margin:0 auto;">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <div>
            <h1 style="font-size:1.4rem;font-weight:800;color:var(--navy-darkest);margin:0;">Buat Pengumuman</h1>
            <p style="font-size:0.82rem;color:#64748b;margin:2px 0 0;">Pengumuman akan tampil di dashboard siswa</p>
        </div>
    </div>

    {{-- Form --}}
    <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;padding:28px;">
        <form action="{{ route('guru-bk.pengumuman.store') }}" method="POST">
            @csrf
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
                ...
                <div>
                    ...
                    <select name="kategori"
                        ...
                        <option value="pribadi_sosial" {{ old('kategori') === 'pribadi_sosial' ? 'selected' : '' }}>Pribadi & Sosial</option>
                        <option value="belajar" {{ old('kategori') === 'belajar' ? 'selected' : '' }}>Belajar & Akademik</option>
                        <option value="karir" {{ old('kategori') === 'karir' ? 'selected' : '' }}>Karir & Masa Depan</option>
                        <option value="info_umum" {{ old('kategori') === 'info_umum' ? 'selected' : '' }}>Info Umum</option>
                    </select>
                    @error('kategori')
                        <p style="font-size:0.76rem;color:#ef4444;margin-top:4px;">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="target"
                        ...
                        <option value="semua" {{ old('target') === 'semua' ? 'selected' : '' }}>Semua Siswa</option>
                        <option value="kelas_binaan" {{ old('target') === 'kelas_binaan' ? 'selected' : '' }}>Kelas Binaan Saya</option>
                    </select>
                    @error('target')
                        <p style="font-size:0.76rem;color:#ef4444;margin-top:4px;">{{ $message }}</p>
                    @enderror
            ...
            <div style="margin-bottom:20px;">
                ...
                <textarea name="isi" rows="6"
                    ...
                    style="width:100%;padding:10px 14px;border:1px solid {{ $errors->has('isi') ? '#ef4444' : '#e2e8f0' }};border-radius:10px;font-size:0.88rem;font-family:inherit;outline:none;resize:vertical;">{{ old('isi') }}</textarea>
                @error('isi')
                    <p style="font-size:0.76rem;color:#ef4444;margin-top:4px;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:24px;padding:14px;background:#fafafa;border-radius:10px;border:1px solid #f1f5f9;">
                ...
                <span style="font-size:0.76rem;color:#94a3b8;">(pengumuman akan muncul paling atas)</span>
            </div>
