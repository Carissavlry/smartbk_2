...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    table.data thead th { padding: 6px 6px; text-align: left; border: 1px solid #1e293b; font-weight: bold; }
...
<table class="kop-table">
    <tr>
        ...
        <td class="kop-center">
            ...
            @endif
            <div class="kop-alamat">{{ $setting['alamat_sekolah'] ?? '' }}@if(!empty($setting['telp_sekolah'])) &nbsp; Telp: {{ $setting['telp_sekolah'] }} @endif</div>
        </td>
...
<div class="judul"><h2>Laporan Sesi Konseling Individual</h2></div>
<div class="sub-judul">Periode: {{ $dari->format('d F Y') }} — {{ $sampai->format('d F Y') }}</div>

<table class="info-table">
    <tr><td>Guru BK</td><td>:</td><td><strong>{{ $gurubk->name }}</strong></td></tr>
    <tr><td>Kelas</td><td>:</td><td>{{ $kelas ? $kelas->nama : 'Semua Kelas' }}</td></tr>
    <tr><td>Total Sesi</td><td>:</td><td><strong>{{ $konselings->count() }} sesi</strong></td></tr>
...
<table class="data">
    ...
    <tr>
        ...
        <td>{{ $k->kategori ?? '-' }}</td>
        <td>{{ \Illuminate\Support\Str::limit($k->deskripsi_masalah ?? '-', 60) }}</td>
        <td>
...
<div class="footer-cetak">Dicetak otomatis oleh sistem SmartBK — {{ $namaSekolah }} — {{ now()->format('d/m/Y H:i') }}</div>
</body>
</html>