...
<div style="padding:28px 32px;min-height:100vh;background:#f1f5f9;">
    ...
    <div style="margin-bottom:24px;">
        ...
        <p style="font-size:0.82rem;color:#64748b;margin:4px 0 0;">Kelola informasi pribadi dan keamanan akun Anda</p>
    </div>
    ...
    <div style="display:grid;grid-template-columns:320px 1fr;gap:24px;align-items:start;">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.06);overflow:hidden;">
                <div style="background:linear-gradient(135deg,#1e3a5f,#6b1d1d);padding:28px 24px;text-align:center;">
                    ...
                    <p style="font-size:0.78rem;color:rgba(255,255,255,0.75);margin:4px 0 0;">NIP: {{ $user->nip ?? '-' }}</p>
                    <span style="display:inline-block;margin-top:8px;background:rgba(255,255,255,0.2);color:white;font-size:0.72rem;font-weight:700;padding:3px 12px;border-radius:20px;border:1px solid rgba(255,255,255,0.3);">
                        Guru BK
                    </span>
                </div>
                <div style="padding:18px 20px;display:flex;flex-direction:column;gap:10px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8fafc;border-radius:8px;">
                        <span style="font-size:0.75rem;color:#64748b;">Kelas Binaan</span>
                        <span style="font-size:0.8rem;font-weight:700;color:#0f172a;">
                            @forelse($kelas as $k)
                                {{ $k->nama }}@if(!$loop->last), @endif
                            @empty
                                -
                            @endforelse
                        </span>
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8fafc;border-radius:8px;">
                        <span style="font-size:0.75rem;color:#64748b;">Email</span>
                        <span style="font-size:0.78rem;font-weight:600;color:#0f172a;">{{ Str::limit($user->email,22) }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8fafc;border-radius:8px;">
                        <span style="font-size:0.75rem;color:#64748b;">No. HP</span>
                        <span style="font-size:0.8rem;font-weight:600;color:#0f172a;">{{ $user->no_hp ?? '-' }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8fafc;border-radius:8px;">
                        <span style="font-size:0.75rem;color:#64748b;">Pendidikan</span>
                        <span style="font-size:0.8rem;font-weight:600;color:#0f172a;">{{ $user->pendidikan_terakhir ?? '-' }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8fafc;border-radius:8px;">
                        <span style="font-size:0.75rem;color:#64748b;">Mulai Bertugas</span>
                        <span style="font-size:0.8rem;font-weight:600;color:#0f172a;">{{ $user->tahun_mulai_bertugas ?? '-' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.06);padding:18px 20px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    ...
                    <div style="text-align:center;padding:12px 8px;background:linear-gradient(135deg,#dcfce7,#bbf7d0);border-radius:10px;border:1px solid #86efac;">
                        <p style="font-size:1.5rem;font-weight:800;color:#15803d;margin:0;">{{ $totalHomeVisit }}</p>
                        <p style="font-size:0.68rem;color:#15803d;margin:2px 0 0;font-weight:600;">Home Visit</p>
                    </div>
                </div>
            </div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.06);overflow:hidden;">
                ...
                <form method="POST" action="{{ route('guru-bk.profil.update') }}" enctype="multipart/form-data" style="padding:24px;">
                    @csrf
                    @method('PUT')
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                        ...
                        <div>
                            <label style="display:block;font-size:0.78rem;font-weight:700;color:#374151;margin-bottom:6px;">Pendidikan Terakhir</label>
                            <input type="text" name="pendidikan_terakhir" value="{{ old('pendidikan_terakhir',$user->pendidikan_terakhir ?? '') }}"
                                style="width:100%;padding:10px 14px;border:1px solid #e2e8f0;border-radius:10px;font-size:0.85rem;color:#0f172a;box-sizing:border-box;"
                                placeholder="Contoh: S1 Bimbingan Konseling"
                                onfocus="this.style.borderColor='#1e3a5f'" onblur="this.style.borderColor='#e2e8f0'">
                        </div>

                        <div>
                            <label style="display:block;font-size:0.78rem;font-weight:700;color:#374151;margin-bottom:6px;">Tahun Mulai Bertugas</label>
                            <input type="number" name="tahun_mulai_bertugas" value="{{ old('tahun_mulai_bertugas',$user->tahun_mulai_bertugas ?? '') }}"
                                style="width:100%;padding:10px 14px;border:1px solid #e2e8f0;border-radius:10px;font-size:0.85rem;color:#0f172a;box-sizing:border-box;"
                                placeholder="Contoh: 2015" min="1970" max="{{ date('Y') }}"
                                onfocus="this.style.borderColor='#1e3a5f'" onblur="this.style.borderColor='#e2e8f0'">
                        </div>
                        ...
                        {{-- READ ONLY --}}
                        <div>
                            <label style="display:block;font-size:0.78rem;font-weight:700;color:#374151;margin-bottom:6px;">NIP</label>
                            ...
                            <p style="font-size:0.7rem;color:#94a3b8;margin-top:4px;">NIP tidak dapat diubah</p>
                        </div>

                        <div>
                            <label style="display:block;font-size:0.78rem;font-weight:700;color:#374151;margin-bottom:6px;">Kelas Binaan</label>
                            <input type="text" value="{{ $kelas->pluck('nama')->join(', ') ?: '-' }}" disabled
                                style="width:100%;padding:10px 14px;border:1px solid #e2e8f0;border-radius:10px;font-size:0.85rem;color:#94a3b8;background:#f8fafc;box-sizing:border-box;cursor:not-allowed;">
                            <p style="font-size:0.7rem;color:#94a3b8;margin-top:4px;">Dikelola oleh Admin</p>
                        </div>
            ...
            <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.06);overflow:hidden;">
                ...
                <div style="padding:20px 24px;">
                    ...
                    @if($user->google_id)
                        {{-- Badge sukses setelah baru connect --}}
                        @if(session('success_google'))
                            <div style="display:flex;align-items:flex-start;gap:10px;padding:14px 16px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;margin-bottom:16px;">
                                <svg style="width:20px;height:20px;flex-shrink:0;margin-top:1px;" fill="none" stroke="#16a34a" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <span style="font-size:0.85rem;color:#15803d;font-weight:600;">{{ session('success_google') }}</span>
                            </div>
                        @else
                            {{-- Badge permanen setelah reload --}}
                            <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;margin-bottom:8px;">
                                <svg style="width:20px;height:20px;flex-shrink:0;" fill="none" stroke="#16a34a" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <div>
                        ...
                        <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;margin-bottom:14px;">
                            <svg style="width:16px;height:16px;flex-shrink:0;" fill="none" stroke="#3b82f6" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <span style="font-size:0.78rem;color:#1d4ed8;">Anda dapat login menggunakan <strong>NIP</strong> atau <strong>Akun Google</strong></span>
                        </div>

                        <form method="POST" action="{{ route('guru-bk.profil.google.disconnect') }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                    ...
                    @else
                        ...
                        <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:14px;">
                            <svg style="width:20px;height:20px;flex-shrink:0;" fill="none" stroke="#94a3b8" viewBox="0 0 24 24">
            ...
            <div style="background:white;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.06);overflow:hidden;">
                ...
                <form method="POST" action="{{ route('guru-bk.profil.password') }}" style="padding:24px;">
                    @csrf
                    @method('PUT')
