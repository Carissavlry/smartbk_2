...
@section('content')
<style>
.chat-wrap { display:flex; flex-direction:column; height:calc(100vh - 180px); background:#fff; border-radius:16px; box-shadow:0 4px 24px rgba(2,16,36,0.12); overflow:hidden; }
...
.back-btn { display:inline-flex; align-items:center; gap:6px; color:#64748b; font-size:0.82rem; text-decoration:none; padding:6px 10px; border-radius:8px; transition:background .15s; }
.back-btn:hover { background:#f1f5f9; color:#052659; }
...
/* SURAT PERINGATAN BUBBLE */
.bubble-surat { background:#fff7ed; border:1px solid #fed7aa; border-radius:12px; padding:12px 16px; max-width:320px; }
.bubble-surat.mine { background:linear-gradient(135deg,#052659,#021024); border-color:#2d5a8e; }
...
.bubble-surat.mine .surat-label { color:#fbbf24; }
.surat-title { font-size:0.85rem; font-weight:700; color:#1e293b; margin-bottom:4px; }
.bubble-surat.mine .surat-title { color:#fff; }
.surat-meta { font-size:0.75rem; color:#78716c; }
.bubble-surat.mine .surat-meta { color:#cbd5e1; }
...
{{-- BACK --}}
<a href="{{ route('guru-bk.chat.index') }}" class="back-btn" style="margin-bottom:14px;display:inline-flex;">
...
<div class="chat-wrap">
    ...
    <div class="chat-header">
        ...
        <div class="chat-header-info">
            <div class="chat-header-name">{{ $siswa->name }}</div>
            <div class="chat-header-sub">{{ $siswa->kelas->nama_kelas ?? '-' }} &middot; {{ $siswa->nis ?? '-' }}</div>
        </div>
    </div>
    ...
    <div class="chat-messages" id="chatMessages">
        ...
        @else
            ...
            @foreach($messages as $msg)
                @php
                    $msgDate = $msg->created_at->setTimezone('Asia/Jakarta')->format('d F Y');
                ...
                <div class="bubble-wrap {{ $isMine ? 'mine' : 'theirs' }}">
                    ...
                    <div class="bubble-surat {{ $isMine ? 'mine' : '' }}">
                        ...
                        <div style="display:flex;gap:6px;flex-wrap:wrap;">
                            <a href="{{ route('guru-bk.surat-peringatan.show', $msg->suratPeringatan) }}"
                            style="display:inline-flex;align-items:center;gap:4px;font-size:0.72rem;font-weight:600;padding:5px 10px;border-radius:7px;text-decoration:none;background:rgba(255,255,255,0.15);color:{{ $isMine ? '#fff' : '#052659' }};border:1px solid {{ $isMine ? 'rgba(255,255,255,0.3)' : '#bfdbfe' }};">
                            ...
                            <a href="{{ route('guru-bk.surat-peringatan.pdf', $msg->suratPeringatan) }}"
                            target="_blank"
                            style="display:inline-flex;align-items:center;gap:4px;font-size:0.72rem;font-weight:600;padding:5px 10px;border-radius:7px;text-decoration:none;background:{{ $isMine ? '#b91c1c' : '#dc2626' }};color:#fff;border:none;">
    ...
    <div class="chat-input-wrap">
        ...
        @endif
        <form class="chat-input-form" method="POST" action="{{ route('guru-bk.chat.send', $siswa) }}">
            @csrf
